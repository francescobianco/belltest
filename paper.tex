...
\begin{verbatim}
a = experiment_A.measure(setting_A, lambda)
b = experiment_B.measure(setting_B, lambda)
\end{verbatim}

where $\lambda$ represents a shared hidden variable or information channel, and experiment_A and experiment_B are two replicated instances exposing the same public measurement method.
...
class Experiment:
    ...
    def measure(self, setting, hidden):
        return self.rule(self.name, setting, hidden)

def threshold_rule(instance_name, setting, hidden):
    thresholds = {
        "alice": (0.25, 0.70),
        "bob": (0.40, 0.60),
    }
...
def run_chsh(n_trials=10000):
    ...
    for _ in range(n_trials):
        ...
        for a_set, b_set in settings:
            a = alice.measure(a_set, hidden)
            b = bob.measure(b_set, hidden)
